...
function (_PIXI$Application) {
  ...
  function App(markus, data) {
    ...
    _this.w = data.props.w || 1920;
    _this.h = data.props.h || 700;
    data.presets.length && _this.stage.addChild.apply(_this.stage, markus.activatePresets(data.presets));
  ...
  _createClass(Markus, [{
    ...
    value: function load(onReady) {
      ...
      PIXI.loader.add(this.filepath).load(function (loader, res) {
        ...
        _this.roots.push(_this.activatePresets(_this.presets));
...
function () {
  ...
  _createClass(MarkusParser, [{
    ...
    value: function getProps(line) {
      var res = {};
      var props = line.replace(/ /g, '').match(/(\w+)=([\w\.]+)/g);
